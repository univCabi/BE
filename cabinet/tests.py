...
from cabinet.models import cabinet_histories, cabinets, users
...
class CabinetBlockchainTest(TestCase):
    def setUp(self):
        # 테스트용 데이터 셋업
        self.building = buildings.objects.create(
            name=BuildingNameEnum.가온관,
        ...
        self.cabinet = cabinets.objects.create(
            building_id=self.building,
            cabinet_number=101,
            status="AVAILABLE",
            payable="FREE"
    ...
    def test_cabinet_history_blockchain_creation(self):
        """새 캐비넷 히스토리 생성 시 블록체인 기록 생성 테스트"""
        # 캐비넷 히스토리 생성
        history = cabinet_histories.objects.create(
            user_id=self.user,
            cabinet_id=self.cabinet,
            expired_at=timezone.now() + timezone.timedelta(days=30)
        )
        
        # 로그를 통해 확인 (실제 블록체인 기록은 개발 모드에서 수행되지 않음)
        self.assertIsNotNone(history)
        # 여기서는 signal이 오류 없이 실행되었는지만 간접적으로 확인